    ...
    // --- WAVE 1.5: Streaming Ice ---
    {
        ...
        action: (scene) => {
            for (let i = 0; i < 15; i++) {
                ((idx) => {
                    setTimeout(() => {
                        const e = new Enemy(scene.game, (idx % 2 === 0 ? 20 : (scene.game.playAreaWidth || 448) - 20), -20);
                        e.color = '#fff';
                        e.setPattern((enemy, dt, t) => {
                            enemy.y += 80 * dt;
    ...
    // --- WAVE 2: More Fairies ---
    {
        ...
        action: (scene) => {
            ...
            for (let i = 0; i < 16; i++) {
                ((idx) => {
                    setTimeout(() => {
                        const e = new Enemy(scene.game, (idx % 2 === 0 ? 0 : (scene.game.playAreaWidth || 448)), 50 + idx * 20);
                        e.color = '#fff';
                        e.setPattern((enemy, dt, t) => {
